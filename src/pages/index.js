...
const addUser = (e) => {
	...
	let user = {
		...
		additionalPhone: document.querySelector('.forms__num-input-additional').value,
		city: document.querySelector('.selector-city').value,
		reason: document.querySelector('.forms__select-reason').value,
...
document.querySelector('.forms-second-stage__btn-next').addEventListener('click', addUser);

const elementsContainer = document.querySelector('.forms-third-stage__list-container');
...
function getItem(item) {
	...
	const addReason = allP.querySelector('.reason');
	...
	addReason.textContent = 'Причина: ' + item.reason;
...
function addEl() {
	...
	elementsContainer.prepend(...page);
}
//  reset array
function removeEl() {
	users = [];
...
const checkInfoBtn = document.querySelector('.forms-second-stage__btn-next').addEventListener('click', () => {
	addEl();
});
...
const popupCloseBtn = document.querySelector('.popup__btn').addEventListener('click', () => {
...
const lastBtn = document.querySelector('.forms-fourth-stage__btn-finish').addEventListener('click', () => {
	resetInputs();
	backToPrimaryStep();
});

// send data to local storage send btn
const sendBtn = document.querySelector('.send').addEventListener('click', () => {
	addUserToLocalStorage();
	removeEl();
});

// forEach cancel btn and add event to close delete array and open popup when click cancel

const cancelBtn = document.querySelectorAll('.forms__btn-cancel').forEach((btn) => {
	btn.addEventListener('click', () => {
		openPopupCancel();
		resetInputs();
		backToPrimaryStep();
